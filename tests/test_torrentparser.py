...
class TestTorrentParse(unittest.TestCase):
    '''
    Unit tests for TorrentParser. Uses meta-data stored in data_torrents_info in data director
    to verify.
    '''
    def setUp(self):
        ...
        if not os.path.exists(test_data_dir):
            raise Exception("Can't find test data dir. Tried: %r" % (test_data_dir,))
        self.test_data_dir = test_data_dir

    def test_parsercreation_invalidtype_file_input(self):
        ''' Test invalid inputs while creating parser object. '''
        self.assertRaises(IOError, TorrentParser, None)
        self.assertRaises(IOError, TorrentParser, 123)
